...
#define SOLE_VERSION                                                                                    \
    "1.0.3" /* (2022/01/17): Merge fixes by @jasonwinterpixel(emscripten) + @jj-tetraquark(get_any_mac) \
...
inline std::string printftime(uint64_t timestamp_secs = 0, const std::string &locale = std::string())
{
    ...
    try
    {
        ...
        std::tm     tm;
        $msvc(localtime_s(&tm, &t);) $melse($windows(tm = *localtime(&t);) $welse(localtime_r(&t, &tm);))

            std::stringstream ss;
        $melse(std::locale lc(locale.c_str()); ss.imbue(lc);) ss << std::put_time(&tm, "\"%c\"");
                ...
                $linux({
                    ...
                    for (struct ifaddrs *ifap = ifaphead; ifap; ifap = ifap->ifa_next)
                    {
                        ...
                        if (std::strcmp("lo", ifap->ifa_name) == 0)
                            continue; // loopback address is zero
