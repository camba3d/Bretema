...
#include <string>   // abort
#include <vector>   // abort
...
static void SetupVulkan(const char **extensions, uint32_t extensions_count)
{
    ...
    // for (auto const &item : available)
    // {
    //     std::cout << "AAAAAAAA => " << std::string(item.layerName);
    // }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (g_SwapChainRebuild)
        {
            ...
            if (width > 0 && height > 0)
            {
                ...
                ImGui_ImplVulkanH_CreateOrResizeWindow(
                    g_Instance, g_PhysicalDevice, g_Device, &g_MainWindowData, g_QueueFamily, g_Allocator, width, height, g_MinImageCount);
